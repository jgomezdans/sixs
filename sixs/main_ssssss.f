      subroutine main_ssssss( x, Lout, xa, xb, xc, verbose, quad_info,
     c anglem, rp )
c
c pass the state vector x
c which we will unpack below in place of the 
c usual input form
c Lout,xa,xb,xc will return the pixel radiance and correction parameters
c
      logical showout ! Set true to produce detailed output to screen
      integer counter,debug,kjploop, quad_info
      integer ibignumber,verbose
      parameter (ibignumber=1000)
      double precision x(ibignumber)
      double precision Lout
...
c****************************************************************************c
	...
	real spdpray,spdpaer,spdptot
        ...
        real fophst,pizerr,pizert,xrad
        double precision xa,xb,xc
...
c***********************************************************************
      ...
      common/aeroprof/alt_z(0:nt_p_max),taer_z(0:nt_p_max),
     &taer55_z(0:nt_p_max),num_z
...
c***********************************************************************
     ...
     s x3(4),cij(4),rsunph(50),nrsunph(50),icp,irsunph
...
! Some variables are not initialised before use later in the code
      idirec=0
      rufet=0. 
      rqfet=0.
! End KJP Initialisations
      angmu(1) = 85.0d0
      do j = 2, 10
        angmu(j) = 80.d0 - (j-2.0d0)*10.d0
      end do
      do j = 1, 13
        angphi(j) = 0.0d0 + 30.0d0*j
      end do
...
c Gaussian weights (weightm) and abscissa (anglem) for evaluating the integral
c between -1 and 1 in mu2 steps
      call gauss(0.,pi2,rp,gp,np)
c Gaussian weights (gp) and abscissa (rp) for evaluating the integral
c between 0 and 2pi (?) in np steps
      mum1=mu-1
      do 581 j=-mum1,-1
       k=mu+j
       rm(-j-mu)=anglem(k)
...
c      read(iread,*) asol,phi0,avis,phiv,month,jday
      ...
      goto 22
c
c 1001 read(iread,*) month,jday,tu,nc,nl
 1001  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     1            asol,phi0,avis,phiv,xlon,xlat)
      goto 22
c 1002 read(iread,*) month,jday,tu,nc,nl
 1002  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     1           asol,phi0,avis,phiv,xlon,xlat)
      ...
      goto 22
c 1003 read(iread,*) month,jday,tu,nc,nl
 1003  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     1           asol,phi0,avis,phiv,xlon,xlat)
      goto 22
c 1004 read(iread,*) month,jday,tu,nc,xlonan,hna
 1004  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     1            asol,phi0,avis,phiv,xlon,xlat)
      goto 22
c 1005 read(iread,*) month,jday,tu,nc,xlonan,hna
 1005  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     1            asol,phi0,avis,phiv,xlon,xlat)
      goto 22
c 1006 read(iread,*) month,jday,tu,xlon,xlat
 1006  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
     ...
     a            asol,phi0,avis,phiv)
      goto 22
c 1007 read(iread,*) month,jday,tu,xlon,xlat
 1007  month = x(counter)
       counter = counter + 1
       jday = x(counter)
       counter = counter + 1
       tu = x(counter)
...
c         7  stratospheric     )  according to Russels model          c
c                                                                      c
...
c      read(5,*) num_z
       num_z = nint(x(counter))
       counter = counter + 1

      do i=0,num_z-1
       height_z(num_z-i) = x(counter)
       counter = counter + 1
       taer55_z(num_z-i) = x(counter)
       counter = counter + 1
       iaer = nint(x(counter))
       counter = counter + 1
c       read(5,*) height_z(num_z-i),taer55_z(num_z-i),iaer
c Lewis comment: this is surely wqrong ...
c iaer isnt used in teh loop, so why read it here?
...
c      if(iaer.eq.4) read(iread,*) (c(n),n=1,4)
      if(iaer.eq.4) then
          c(1)  = x(counter)
          counter = counter + 1
          c(2)  = x(counter)
...
c     read(iread,*) rmin,rmax,icp
      do i=1,icp
        ...
        cij(i)=x(counter)
        counter=counter+1
c       read(5,*)x1(i),x2(i),cij(i)
        do kjploop=1,20
           rn(kjploop,i)=x(counter)
           counter=counter+1
        enddo
c       read(5,*)(rn(l,i),l=1,20)
        do kjploop=1,20
           ri(kjploop,i)=x(counter)
           counter=counter+1
        enddo
c       read(5,*)(ri(l,i),l=1,20)
      enddo
        do i=1,icp
         cij_out(i)=cij(i)
        enddo
...
c      read(iread,*) rmin,rmax
      ...
      x3(1)=x(counter)
      counter=counter + 1
c      read(iread,*) x1(1),x2(1),x3(1)
      do kjploop=1,20
         rn(kjploop,1)=x(counter)
         counter=counter+1
      enddo
c      read(5,*)(rn(l,1),l=1,20)
      do kjploop=1,20
         ri(kjploop,1)=x(counter)
         counter=counter+1
      enddo
c      read(5,*)(ri(l,1),l=1,20)
      go to 49
   45 rmin=x(counter)
      counter=counter + 1
      rmax=x(counter)
...
c       read(5,*)iaerp
       iaerp=nint(x(counter))
       counter=counter+1
       if (iaerp.eq.1) FILE='tmp0 ' !Hardwire for the Example test
c     read(5,'(A80)')FILE
       i1=index(FILE,' ')-1
       FILE2=FILE(1:I1)//'.mie'
       i2=index(FILE2,' ')-1
...
c                                                                      c
c         1  enter wlinf, wlsup and users filter function s(lambda)   c
...
c       119  ALI     band 1p          (0.4225-0.4625)                  c
c       120  ALI     band 1           (0.4325-0.550)                   c
c       121  ALI     band 2           (0.500-0.630)                    c
c       122  ALI     band 3           (0.5755-0.730)                   c
c       123  ALI     band 4           (0.7525-0.8375)                  c
c       124  ALI     band 4p          (0.8025-0.935)                   c         c       125  ALI     band 5p          (1.130-1.345)                    c
c       126  ALI     band 5           (1.470-1.820)                    c         c       127  ALI     band 7           (1.980-2.530)                    c         c       128  ASTER   band 1           (0.485-0.6425)                   c
c       129  ASTER   band 2           (0.590-0.730)                    c
c       130  ASTER   band 3n          (0.720-0.9075)                   c
c       131  ASTER   band 3b          (0.720-0.9225)                   c
c       132  ASTER   band 4           (1.570-1.7675)                   c
c       133  ASTER   band 5           (2.120-2.2825)                   c
c       134  ASTER   band 6           (2.150-2.295)                    c
c       135  ASTER   band 7           (2.210-2.390)                    c
c       136  ASTER   band 8           (2.250-2.440)                    c
c       137  ASTER   band 9           (2.2975-2.4875)                  c
c       138  ETM     band 1           (0.435-0.52) 		                 c         c       139  ETM     band 2           (0.5-0.6225)                     c
c       140  ETM     band 3           (0.615-0.7025)                   c
c       141  ETM     band 4           (0.74-0.9125)                    c
c       142  ETM     band 5           (1.51-1.7875)                    c
c       143  ETM     band 7           (2.015-2.3775)                   c
c       144  HYPBLUE band 1           (0.4375-0.500)                   c
c       145  HYPBLUE band 2           (0.435-0.52)                     c
c       146  VGT     band 1           (0.4175-0.500)                   c
c       147  VGT     band 2           (0.5975-0.7675)                  c
c       148  VGT     band 3           (0.7325-0.9575)                  c
c       149  VGT     band 4           (1.5225-1.800)                   c
c       150  VIIRS   band M1          (0.4025-0.4225)                  c
c       151  VIIRS   band M2          (0.4350-0.4550)                  c 
c       152  VIIRS   band M3          (0.4775-0.4975)                  c 
c       153  VIIRS   band M4          (0.5450-0.5650)                  c 
c       154  VIIRS   band M5          (0.6625-0.6825)                  c 
c       155  VIIRS   band M6          (0.7375-0.7525)                  c 
c       156  VIIRS   band M7          (0.8450-0.8850)                  c 
c       157  VIIRS   band M8          (1.2300-1.2500)                  c 
c       158  VIIRS   band M9          (1.3700-1.3850)                  c 
c       159  VIIRS   band M10         (1.5800-1.6400)                  c 
c       160  VIIRS   band M11         (2.2250-2.2750)                  c
c       161  VIIRS   band M12         (3.6100-3.7900)                  c 
c       162  VIIRS   band I1          (0.6000-0.6800)                  c 
c       163  VIIRS   band I2          (0.8450-0.8850)                  c 
c       164  VIIRS   band I3          (1.5800-1.6400)                  c 
c       165  VIIRS   band I4          (3.5500-3.9300)                  c
...
c   17 read(iread,*) wlinf,wlsup
 17   wlinf = x(counter)
      ...
      go to 19
c 1600 read(iread,*) wlinf,wlsup
 1600  wlinf = x(counter)
       counter = counter + 1
       wlsup = x(counter)
       counter = counter + 1
...
c       159     VIIRS   band            (150,165)
  ...
  110 wlinf=x(counter)
      ...
      wlsup=x(counter)
      counter=counter+1
c     read(iread,*) wlinf,wlsup
      iinf=(wlinf-.25)/0.0025+1.5
      isup=(wlsup-.25)/0.0025+1.5
      do 1113 ik=iinf,isup
       s(ik)=0.
 1113 continue
      do kjploop=iinf,isup
        s(kjploop)=x(counter)
        counter=counter+1
      enddo
c      read(iread,*) (s(i),i=iinf,isup)
      goto 20
  111 call meteo
      go to 19
  ...
  152 call gli(iwave-88)
      goto 19
  153 call ali(iwave-118)
      goto 19 
  154 call aster(iwave-127)
      goto 19
  155 call etm(iwave-137)
      goto 19
  156 call hypblue(iwave-143)
      goto 19
  157 call vgt(iwave-145)
      goto 19
  159 call viirs(iwave-149)
      goto 19 
...
c         9   Kuusks multispectral CR model                           c
c             Reference:                                               c
...
c      read(iread,*) inhomo
      inhomo = nint(x(counter))
      counter = counter + 1

      if(inhomo) 30,30,31

c  30  read(iread,*) idirec
 30    idirec = nint(x(counter))
      counter = counter + 1
...
c**********************************************************************c
  ...
  23  do 900 k=1,13
         do kjploop=1,10
           brdfdats(10-kjploop+1,k)=1.0d0*real(x(counter))
           counter=counter+1
           
         enddo
c        read(iread,*) (brdfdats(10-j+1,k),j=1,10)
      
  900 continue
      do 901 k=1,13
         do kjploop=1,10
           brdfdatv(10-kjploop+1,k) = 1.0d0*x(counter)
           counter=counter+1
           
         enddo
c        read(iread,*) (brdfdatv(10-j+1,k),j=1,10)
  901 continue
      albbrdf=real(x(counter))
      counter=counter+1
      rodir=real(x(counter))
      counter=counter+1
c      read(iread,*) albbrdf
c      read(iread,*) rodir
      rm(-mu)=phirad
      rm(mu)=xmuv
      rm(0)=xmus
...
c**********************************************************************c
  24  if(ibrdf.eq.1) then
        ...
        par4=x(counter)
        counter=counter+1
c        read(iread,*) par1,par2,par3,par4
...
c**********************************************************************c
c     brdf from verstraete et als model                               c
c**********************************************************************c
      if(ibrdf.eq.2) then
         options(3)=nint(x(counter))
         counter=counter+1
         options(4)=nint(x(counter))
         counter=counter+1
         options(5)=nint(x(counter))
         counter=counter+1
c        read(iread,*) (options(i),i=3,5)
        options(1)=1
        options(2)=1
         struct(1)=(x(counter))
         counter=counter+1
         ...
         optics(3)=(x(counter))
         counter=counter+1
c        read(iread,*) (struct(i),i=1,4)
c        read(iread,*) (optics(i),i=1,3)
...
c**********************************************************************c
c     brdf from Roujean et als model                                  c
c**********************************************************************c
      if(ibrdf.eq.3) then
         par1=(x(counter))
         counter=counter+1
...
c**********************************************************************c
c     brdf from walthall et als model
c**********************************************************************c
      if(ibrdf.eq.4) then
         par1=(x(counter))
         counter=counter+1
...
c**********************************************************************c
      if(ibrdf.eq.6) then
         ...
         pcl=(x(counter))
         counter=counter+1
c        read(iread,*) pws,phi_wind,xsal,pcl
...
c**********************************************************************c
      if(ibrdf.eq.7) then
         ...
         pRs=(x(counter))
         counter=counter+1
c        read(iread,*) pild,pihs
c        read(iread,*) pxLt,pc
c        read(iread,*) pRl,pTl,pRs
...
c
c**********************************************************************c
c     brdf from kuusks msrm model                                     c
c**********************************************************************c
      if(ibrdf.eq.9) then
c         read(iread,*) uli,eei,thmi,sli
c         read(iread,*) cabi,cwi,vaii,rnci,rsl1i
         uli = x(counter)
         counter = counter + 1
         eei = x(counter)
         counter = counter + 1
...
c**********************************************************************c
      ...
      do 83 j=1,np
        ...
        do 84 k=1,mu-1
          rdown=xlmus(-k,j)
          rdir=brdfintv(k,j)
c          print*, "brdfintv", k, j, brdfintv(k,j)
...
c**********************************************************************c
      ...
      do 85 j=1,np
        ...
        do 86 k=1,mu-1
          rdown=xlmuv(-k,j)
          rdir=brdfints(k,j)
c          print*, "brdfints", k, j, brdfints(k,j)
          rob=rob+rdown*rdir*rm(k)*gb(k)
          xnor=xnor+rdown*rm(k)*gb(k)
   86   continue
...
c     read(iread,*) igroun
  ...
  29  nwlinf=(x(counter))
      ...
      nwlsup=(x(counter))
      counter=counter+1
c      read(iread,*) nwlinf,nwlsup
      niinf=(nwlinf-.25)/0.0025+1.5
      nisup=(nwlsup-.25)/0.0025+1.5
      do kjploop=niinf,nisup
         rocl(kjploop)=x(counter)
         counter=counter+1
      enddo   
c      read(iread,*) (rocl(i),i=niinf,nisup)
      goto 36
...
c**********************************************************************c
 31   igrou1=nint(x(counter))
      counter=counter+1
      igrou2=nint(x(counter))
      counter=counter+1
      rad=(x(counter))
      counter=counter+1

c     read(iread,*) igrou1,igrou2,rad
      if(igrou1) 59,60,63
  59  do kjploop=iinf,isup
         rocl(kjploop)=x(counter)
         counter=counter+1
      enddo
c     read(iread,*) (rocl(i),i=iinf,isup)
      goto 61
  60  roc=x(counter)
      counter=counter+1
c     read(iread,*) roc
  ...
  66  do kjploop=iinf,isup
         roel(kjploop)=x(counter)
         counter=counter+1
      enddo
c     read(iread,*) (roel(i),i=iinf,isup)
      goto 34
  62  roe=x(counter)
      counter=counter+1
c     read(iread,*) roe
...
c     KJP debug iwave inhomo output
!       write(6,*) iwave,inhomo
       irop=0

       irop=nint(x(counter))
       counter=counter+1
c       read(iread,*,end=37) irop
...
c --- atmospheric model ----
      if (showout) write(iwr, 1119)
      if(idatm-7)226,227,228
  228 if (showout) write(iwr, 1281)uw,uo3
      goto 219
  227 if (showout) write(iwr, 1272)
      do 229 i=1,34
        if (showout) write(iwr, 1271)z(i),p(i),t(i),wh(i),wo(i)
  229 continue
      goto 219
  226 if (showout) write(iwr, 1261)atmid(idatm+1)
 
c --- aerosols model (type) ----

219    if (showout) write(iwr,5550)
       if(iaer.eq.0) then
        if (showout) write(iwr, 5554)
        goto 1112
       endif
...
c --- ground reflectance (type and spectral variation) ----
  ...
  264   continue
        ...
        if (showout) write(iwr, 169)rad
        igroun=igrou1
        ro=rocave
        if (showout) write(iwr, 170)
        goto 261
 
  262   igroun=igrou2
        ro=roeave
        if (showout) write(iwr, 171)
        goto 261
        
  260   if (showout) write(iwr, 168)
  261   if (igroun.gt.0) then 
           if (showout) write(iwr, reflec(igroun+3))ro
        endif
        if (igroun.gt.0) goto 158
        if(igroun.eq.-1) then 
           if (showout) write(iwr, reflec(1))ro
        endif
 ...
 158    isort=isort+1
     ...
     *    ,(ibrdf+1)
 2000    if (showout) write(iwr, 190)
         if (showout) write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         goto 2009
 2001    if (showout) write(iwr, 191)par1,par2,par3,par4
         if (showout) write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         goto 2009
 2002    if (showout) then 
            write(iwr, 192)optics(1),struct(1),struct(2)
     ...
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         ...
         goto 2009
 2003    if (showout) then 
            write(iwr, 193)par1,par2,par3
            write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
 2004    if (showout) then 
            write(iwr, 194)par1,par2,par3,par4
            write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
 2005    if (showout) then 
            write(iwr, 195)par1,par2
            write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
 2006    if (showout) then 
            write(iwr, 196)pws,phi_wind,xsal,pcl
            write(iwr,500) rfoamave,rwatave,rglitave
            write(iwr, 187)
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
 2007    if (showout) then
         write(iwr, 205) pRl,pTl,pRs,PxLt
         if (pihs.eq.0) then
     ...
     *rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         ...
         goto 2009
 2008    if (showout) write(iwr, 206) par1,par2,par3
         goto 2009
 2010    if (showout) then 
            write(iwr, 210)uli,eei,thmi,sli,cabi,cwi,vaii,rnci,rsl1i
            write(iwr, 187)
     *   rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
 2011    if (showout) then 
            write(iwr, 211)p1,p2,p3
            write(iwr, 187)
     *   rocave,robar1/xnorm1,robar2/xnorm2,albbrdf
         endif !(showout)
         goto 2009
...
c      endif
c END of if debug statement (Lewis)
c**********************************************************************c
c                                                                      c
c                                                                      c
...
c
     ...
     s            swl)
        swl=swl*dsol
        coef=sbor*step*swl
c        write(6,*) coef,sbor,step,swl,wl
C        pause
        coefp=sbor*step
...
c     KJP Debug line
c        write(6,*) ratm1,romix,rorayl,tgtot,tgp1
        ratm3=romix*tgp1
...
c     KJP Debug lines
c        write(6,*)romeas1,ratm1,rsurf,tgtot
c        pause
...
C End Update Look up table	
        ...
        if (iwave.eq.-2) then
          if (showout) write(iwr,1501) 
     &                     wl,tgtot,dtott,utott,astot,ratm2,swl,roc,
     s            sbor,dsol,romeas2
        endif
...
C Case a LUT output is desired      
 ...
 160  continue
        ...
        if (showout) write(iwr, 430 )refet,alumet,tgasm
        if (showout) write(iwr, 431 )refet1,refet2,refet3

      if (ipol.eq.1)then
        rpfet=sqrt(rqfet*rqfet+rufet*rufet)
	plumet=sqrt(qlumet*qlumet+ulumet*ulumet)
	xpol=atan2(rufet,rqfet)*180.0/3.14159/2.
        if (showout) write(iwr, 429 )rpfet,plumet,xpol,rpfet/refet
...
!     return here not stop - KJP 17/5/11
      Lout=ainr(2,3)
...
c                                                                      c
c**********************************************************************c
!Some of these formats have a 1X before t79 which seems to confuse things. 
!The 1X is redundant anyway since we tab to column 79 so removed to tidy output
!KJP 1/6/11
...
!     s F8.2,1X,t79,1h*,/,1h*,t79,1h*) !KJP Edit 1/6/11
     s F8.2,t79,1h*,/,1h*,t79,1h*)
...
!     s36h  Sunglint Model  windspeed,azimuth ,2(F8.3,1X),
     s36h  Sunglint Model  windspeed,azimuth ,F8.3,1X,F8.3,
     s t79,1h*,/,1h*)
...
!  175 format(1h*,10x,22h ground altitude  [km]    ,f6.3,1x,t79,1h*)
  174 format(1h*,10x,22h ground pressure  [mb]    ,1x,f7.2,t79,1h*)
  175 format(1h*,10x,22h ground altitude  [km]    ,f6.3,t79,1h*)
...
!     s       1h*,15x,4(f9.4,1x),t79,1h*)
     s       1h*,15x,3(f9.4,1x),f9.4,t79,1h*)
...
!  180 format(1h*,10x,31h plane  altitude absolute [km] ,f6.3,1x,t79,1h*)
  179 format(1h*,10x,31h plane  pressure          [mb] ,f7.2,t79,1h*)
  180 format(1h*,10x,31h plane  altitude absolute [km] ,f6.3,t79,1h*)
...
!  184 format(1h*,15x,26haerosol opt. thick. 550nm ,f6.3,1x,t79,1h*)
  182 format(1h*,15x,26h ozone content            ,f6.3,t79,1h*)
  183 format(1h*,15x,26h h2o   content            ,f6.3,t79,1h*)
  184 format(1h*,15x,26haerosol opt. thick. 550nm ,f6.3,t79,1h*)
...
! KJP Edit 1/6/11
!     s               25h wv mixed with aerosol : ,f5.3,1x,t79,1h*,/,
     s               25h wv mixed with aerosol : ,f5.3,t79,1h*,/,
     s       1h*,22x,20h wv under aerosol : ,f5.3,t79,1h*)
...
!     s           1x, 3(f8.5,1x),t79,1h*,/,1h*,6x,
     s           1x, 2(f8.5,1x),f8.5,t79,1h*,/,1h*,6x,
