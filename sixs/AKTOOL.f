...
c
     ...
     & vaii, pir
      double precision mu1, mu2, fi
      double precision rm(-mu:mu), rp(np), brdfint(-mu:mu, np)
      save /count/, /soildata/, /aaa/, /ggg/, /ladak/
c
      dimension u1(10), u2(10), a1(10), a2(10)
      common /count/ jl, jj, lg, jg, lf, nnx, n1, n2, u1, u2, a1, a2
...
!       write(6,*) rm(I)
!      enddo
!      write(6,*) 'j, k, acos(rm(j)), rp(k), result'
        ee    = eei
        thm   = thmi*dr
        ul    = uli
        sl    = sli
...
c
        ...
        if ((rlambda .gt. 2500.d0) .or. (rlambda .lt. 404.d0)) then
           print *, 'AKBRDF: wavelength out of range ', wlmoy, rlambda
           print*, ee, thmi, uli, sli, rsl1i, wlmoy,cabi, cwi, vaii
           stop
        endif
c
...
! KJP TEST 6/6/11
!        write(6,*) 'rp(k)=',rp(k),'rm(-mu)=',rm(-mu),'fi=',fi
        tgt1 = tan(th10)
        xx   = tgt1*clx/sl
c
        if (xx .lt. eps) then
...
c
        ...
        brdfint(j, k) = bq
c     KJP tests 1/6/11
!        write(6,*) j,k,180.*acos(rm(j))/pi,180.*rp(k)/pi,bq
c
  2   continue
  1   continue
c     KJP test
